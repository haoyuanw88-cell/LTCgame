...
<div class="shell">
  <aside class="sidebar">
    ...
    <nav class="nav" aria-label="後台模組">
      <button class="active" type="button"><span class="mark"></span>營運儀表板</button>
      <button type="button"><span class="mark"></span>玩家與機構</button>
      <button type="button"><span class="mark"></span>遊戲內容管理</button>
      <button type="button"><span class="mark"></span>認知趨勢分析</button>
      <button type="button"><span class="mark"></span>照護追蹤任務</button>
      <button type="button"><span class="mark"></span>報表匯出</button>
  ...
  <main class="main">
    <header class="topbar">
      <div><div class="eyebrow">LONG-TERM CARE GAME ANALYTICS</div><h1>後台管理系統</h1></div>
      <div class="actions">
        <select class="control" aria-label="資料期間"><option>近 7 天</option><option>全部資料</option></select>
        <select class="control" aria-label="機構篩選"><option>全部機構</option></select>
        <button class="icon-btn" id="refreshButton" type="button" title="重新整理">↻</button>
        <button class="primary-btn" id="exportButton" type="button">匯出報表</button>
      </div>
    </header>
    <section class="grid stats" aria-label="營運摘要">
      <article class="card stat"><div class="label">目前在線人數</div><div class="value" id="online">-</div><div class="delta">最近 5 分鐘心跳</div></article>
      <article class="card stat"><div class="label">累積玩家</div><div class="value" id="players">-</div><div class="delta">跨機構去識別統計</div></article>
    ...
    <section class="grid layout">
      ...
      <aside class="grid">
        <article class="panel">
          <div class="panel-head"><div><h2>近 7 天趨勢</h2><div class="muted">每日完成次數與平均分</div></div><span class="live"><span class="dot"></span><span id="updated">連線中</span></span></div>
        ...
        <article class="panel">
          <div class="panel-head"><div><h2>最近遊戲紀錄</h2><div class="muted">去識別玩家編號</div></div></div>
          <div style="overflow:auto"><table><thead><tr><th>玩家</th><th>遊戲</th><th>分數</th><th>狀態</th></tr></thead><tbody id="recentBody"></tbody></table></div>
        </article>
      </aside>
    </section>
  </main>
...
function renderTrend(data){const chart=byId('trendChart');chart.innerHTML='';const points=data.weeklyTrend||[];byId('trendEmpty').style.display=points.length?'none':'block';const maxSessions=Math.max(...points.map(point=>point.sessions),1);for(const point of points){const col=document.createElement('div');col.className='trend-col';col.innerHTML='<div class="muted hide-sm">'+(point.score||0)+'分</div><div class="trend-bar" title="'+point.sessions+' 筆" style="height:'+((point.sessions||0)/maxSessions*78+8)+'%"></div><div class="trend-label">'+point.date+'</div>';chart.append(col)}}
...
function renderRecent(data){byId('recentBody').innerHTML=(data.recentSessions||[]).map(item=>'<tr><td>'+item.playerId+'<div class="muted">'+item.playedAt+'</div></td><td>'+item.game+'</td><td><strong>'+item.score+'</strong></td><td><span class="pill '+(item.status==='需複核'?'risk':'')+'">'+item.status+'</span></td></tr>').join('')}
function renderAll(data){state.data=data;renderStats(data);renderDomains();renderTrend(data);renderTasks(data);renderGames(data);renderRecent(data)}
